...
if os.path.isfile(priv_key):
    if is_binaray_string(priv_key):
        private_key = from_prikey_file(priv_key, password)
        wallet = KeyWallet.load(private_key)
        wallet.store(f"{priv_key}.json", password)
        print(wallet.get_address())
    else:
        pubkey = from_prikey_file(priv_key, password)
        wallet = KeyWallet.load(pubkey[1:])
        wallet.store(f"{priv_key}.json", password)
        print(wallet.get_address())
else:
